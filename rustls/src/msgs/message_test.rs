...
fn test_read_fuzz_corpus() {
    ...
    for file in fs::read_dir(corpus_dir()).unwrap() {
        ...
        let msg: PlainMessage = OpaqueMessage::read(&mut rd)
            .unwrap()
            .into_plain_message();
        println!("{:?}", msg);
...
fn can_read_safari_client_hello() {
    ...
    let mut rd = Reader::init(bytes);
    let m: OpaqueMessage = OpaqueMessage::read(&mut rd).unwrap();
    println!("m = {:?}", m);
...
fn construct_all_types() {
    ...
    for &bytes in samples.iter() {
        let m: OpaqueMessage = OpaqueMessage::read(&mut Reader::init(bytes)).unwrap();
        println!("m = {:?}", m);
